...
const AppContent: React.FC = () => {
  ...
  const triggerValidateRefine = async (analysisFile?: string) => {
    try {
      ...
      const response = await fetch('/api/validate-refine', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ analysisFile })
      });

      if (response.ok) {
        const result = await response.json();
        console.log('Validation result:', result);
        if (result.validation?.notFound > 0) {
          console.log(`Refined: removed ${result.refined.removedCount} potential hallucinations`);
          console.log(`New analysis file: ${result.refined.analysisFile}`);
        } else {
          console.log('All papers validated successfully');
        }
        fetchHistory(); // Refresh to show new refined files
        return result;
      } else {

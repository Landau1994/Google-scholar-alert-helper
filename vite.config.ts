    ...
    react(),
    {
      ...
      configureServer(server) {
        server.middlewares.use((req, res, next) => {
          ...
          // API: Validate and Refine - triggers paper validation and generates refined files
          if (url === '/api/validate-refine' && req.method === 'POST') {
            let body = '';
            req.on('data', chunk => {
              body += chunk.toString();
            });
            req.on('end', async () => {
              try {
                const { syncFile, analysisFile } = JSON.parse(body || '{}');

                // Dynamic import the validation module
                const { validateAndRefine } = await import('./scripts/validatePaperTitles.ts');

                const syncPath = syncFile ? path.resolve(__dirname, 'synced_emails', syncFile) : undefined;
                const analysisPath = analysisFile ? path.resolve(__dirname, 'synced_emails', analysisFile) : undefined;

                console.log('[API] Running validate-refine...');
                const result = await validateAndRefine(syncPath, analysisPath);

                res.setHeader('Content-Type', 'application/json');
                res.end(JSON.stringify({
                  status: 'success',
                  validation: {
                    totalPapers: result.validation.totalPapers,
                    validated: result.validation.validated,
                    notFound: result.validation.notFound,
                    validationRate: result.validation.validationRate
                  },
                  refined: {
                    originalCount: result.refined.originalCount,
                    refinedCount: result.refined.refinedCount,
                    removedCount: result.refined.removedCount,
                    analysisFile: path.basename(result.refined.refinedAnalysisFile),
                    reportFile: path.basename(result.refined.refinedReportFile)
                  }
                }));
              } catch (error) {
                console.error('[API] validate-refine error:', error);
                res.statusCode = 500;
                res.end(JSON.stringify({ status: 'error', message: (error as Error).message }));
              }
            });
